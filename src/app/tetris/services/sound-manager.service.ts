...
export class SoundManagerService {
  ...
  start() { this.playMusic(0, 3.7202, 3.6224); }
  clear() { this.playMusic(0, 0, 0.7675); }
  fall() { this.playMusic(0, 1.2558, 0.3546); }
  gameOver() { this.playMusic(0, 8.1276, 1.1437); }
  rotate() { this.playMusic(0, 2.2471, 0.0807); }
  move() { this.playMusic(0, 2.9088, 0.1437); }

  private playMusic(when: number, offset: number, duration: number) {
    if (!this.tetrisState.isEnableSound()) { return; }
    this.loadSound().then((source) => {
      if (source) { source.start(when, offset, duration); }
    });
  }

  private loadSound(): Promise<AudioBufferSourceNode> {
    return new Promise((resolve, reject) => {
      if (typeof AudioContext === 'undefined' || typeof XMLHttpRequest === 'undefined') return reject('Not in browser');
