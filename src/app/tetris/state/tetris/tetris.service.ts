...
import { EmptyTile } from '@app/tetris/interface/tile/empty-tile';
...
import { SoundManagerService } from '@app/tetris/services/sound-manager.service';
...
export class TetrisService {
  ...
  moveLeft() {
    if (this.tetrisState.locked() || !this.tetrisState.current()) {
      return;
    }
    this.clearPiece();
  ...
  holdPiece(): void {
    if (this.tetrisState.locked() || !this.tetrisState.canHold() || !this.tetrisState.current()) {
      return;
    }
    this.clearPiece();
  ...
  private get isGameOver() {
    if (!this.tetrisState.current()) return false;
    this.setCurrentPiece(this.tetrisState.current()!.store());
    this.setCurrentPiece(this.tetrisState.current()!.moveDown());
    if (this.isCollidesBottom) {
      return true;
