...
export class MonthViewComponent implements OnInit {
    ...
    isModalOpen = false;
    newEventTitle = '';
    currentDayForEvent: Date | null = null;
    ...
    selectDate(day: Date) { this.selectedDate = day; }
    getDaysInMonth(): Date[] {
        const days = [];
    ...
    closeModal() {
        ...
        this.currentDayForEvent = null;
        this.newEventTitle = '';
    }

    saveEvent() {
        if (this.currentDayForEvent && this.newEventTitle.trim()) {
            const dateString = this.currentDayForEvent.toISOString().split('T')[0];
            this.eventService.addEvent(dateString, this.newEventTitle);
            this.loadEvents();
            this.closeModal();
        }
    }
